...
export const LiveShare = {
  ...
  async join() {
    const serverUrl = (document.getElementById('liveServer').value || this.defaultServer()).trim().replace(/\/$/, '');
    const room = this.normalizeRoom(document.getElementById('liveRoom').value || 'team-ride') || 'team-ride';
    const name = (document.getElementById('liveName').value || 'Rider ' + this.makeId()).trim().slice(0, 32);
    ...
    try {
      ...
      socket.on('webrtc:answer', ({ from, answer }) => { this.handleAnswer(from, answer); });
      socket.on('webrtc:ice', ({ from, candidate }) => { this.handleIce(from, candidate); });
